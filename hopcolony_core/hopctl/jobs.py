...
def create(name: str):
    try:
        ...
        typer.secho(f"Project {name} already exists", err = True, fg=typer.colors.RED)
    ...
    cmd = typer.style(f"hopctl jobs run example --project {name}", fg=typer.colors.YELLOW, bold=True)
...
def run(name: str, project: Optional[str] = ".", skip: Optional[bool] = False):
    ...
    try:
        ...
        typer.secho(f"settings.yaml file not found on project {project}", err = True, fg=typer.colors.RED)
        raise typer.Exit(code=1)
        ...
        with click_spinner.spinner():
            ...
            try:    
                with open(os.devnull, 'w') as devnull:
                    subprocess.check_call([sys.executable, "-m", "pip", "install", *requirements], stdout=devnull)
    ...
    job = job_cls(**args)
    pipelines = [ pipeline.instanciate(job) for pipeline in pipelines ] if pipelines else []
