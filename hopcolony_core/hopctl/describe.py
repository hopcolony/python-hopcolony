...
def index(name: str, cols: str = None, doc: Optional[str] = None):
    ...
    if not doc:
        ...
        if snapshot.success:
            ...
            documents = tabulate([doc.printable(columns) for doc in snapshot.docs], headers=docs.Document.printable_headers(columns))
        ...
        if snapshot.success:
            ...
            typer.secho(f"Document \"{doc}\" not found in index {name}", fg = typer.colors.RED)
...
def bucket(name: str, obj: Optional[str] = None):
    ...
    if not obj:
        ...
        if snapshot.success:
            objects = tabulate([object.printable for object in snapshot.objects], headers=drive.Object.printable_headers)
            typer.echo(objects)
        else:
        ...
        snapshot = client.bucket(name).get()
        if snapshot.success:
            matching = [object for object in snapshot.objects if object.id == obj]
            if not matching:
                typer.secho(f"Object \"{obj}\" not found in index {name}", fg = typer.colors.RED)
                raise typer.Exit()
