...
class HopDriveClient:
    ...
    def get(self, path):
        ...
        resp = self._session.request("GET", self.base_url + path, headers=signDetails.headers)
        resp.raise_for_status()
        return resp

    def put(self, path, bodyBytes):
        signDetails = self.signer.sign("PUT", path, bodyBytes=bodyBytes)
        resp = self._session.request("PUT", self.base_url + path, headers=signDetails.headers, data=bodyBytes)
        resp.raise_for_status()
        return resp
    
    def delete(self, path):
        signDetails = self.signer.sign("DELETE", path)
        resp = self._session.request("DELETE", self.base_url + path, headers=signDetails.headers)
        resp.raise_for_status()
        return resp